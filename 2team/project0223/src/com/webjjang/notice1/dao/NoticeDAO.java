...
public class NoticeDAO {
	...
	// 공지 리스트 -----------------------------
	public List<com.webjjang.notice1.vo.NoticeVO> list() throws Exception{
		
		// 실행한 결과를 저장하는 객체 선언
		List<com.webjjang.notice1.vo.NoticeVO> list = null;
		...
		try {
			...
			System.out.println(DBSQL.NOTICE1_LIST);
			//4. 실행객체 & 데이터 셋팅
			...
			if(rs != null) {
				while(rs.next()) {
					...
					if(list == null) list = new ArrayList<com.webjjang.notice1.vo.NoticeVO>();
					// rs -> vo 저장을 위해 vo 객체 생성
					com.webjjang.notice1.vo.NoticeVO vo = new com.webjjang.notice1.vo.NoticeVO();
	...
	// 공지 글쓰기 -----------------------------
	public int write(com.webjjang.notice1.vo.NoticeVO vo) throws Exception{
		...
		try {
			...
			con = DBInfo.getConnection();
			//3. 쿼리 작성 - DBSQL에 선언됨. - 확인
			System.out.println(DBSQL.NOTICE1_WRITE);
			//4. 실행객체 & 데이터 셋팅
