...
public class NoticeDAO {
	...
	public List<NoticeVO> list() throws Exception{
		...
		try {
			...
			System.out.println(DBSQL.NOTICE2_LIST);
			//4. 실행객체 & 데이터 셋팅
			pstmt = con.prepareStatement(DBSQL.NOTICE2_LIST);
			// 5. 실행
	...
	public NoticeVO view(long no) throws Exception{
		...
		try {
			...
			con = DBInfo.getConnection();
			//3. 쿼리 작성 - DBSQL에 선언됨. - 확인
			System.out.println(DBSQL.NOTICE2_VIEW);
			//4. 실행객체 & 데이터 셋팅
			pstmt = con.prepareStatement(DBSQL.NOTICE2_VIEW);
			pstmt.setLong(1, no);
			// 5. 실행
			rs = pstmt.executeQuery();
	...
	public int write(NoticeVO vo) throws Exception{
		...
		try {
			...
			con = DBInfo.getConnection();
			//3. 쿼리 작성 - DBSQL에 선언됨. - 확인
			System.out.println(DBSQL.NOTICE2_WRITE);
			//4. 실행객체 & 데이터 셋팅
			pstmt = con.prepareStatement(DBSQL.NOTICE2_WRITE);
	...
	public int update(NoticeVO vo) throws Exception{
		...
		try {
			...
			con = DBInfo.getConnection();
			//3. 쿼리 작성 - DBSQL에 선언됨. - 확인
			System.out.println(DBSQL.NOTICE2_UPDATE);
			//4. 실행객체 & 데이터 셋팅
			pstmt = con.prepareStatement(DBSQL.NOTICE2_UPDATE);
			pstmt.setString(1, vo.getTitle());
			pstmt.setString(2, vo.getContent());
			pstmt.setString(3, vo.getStartDate());
			pstmt.setString(4, vo.getEndDate());
	...
	public int delete(long no) throws Exception{
		...
		try {
			...
			con = DBInfo.getConnection();
			//3. 쿼리 작성 - DBSQL에 선언됨. - 확인
			System.out.println(DBSQL.NOTICE2_DELETE);
			//4. 실행객체 & 데이터 셋팅
			pstmt = con.prepareStatement(DBSQL.NOTICE2_DELETE);
			pstmt.setLong(1, no);
			// 5. 실행 - delete -> 결과가 int
			result = pstmt.executeUpdate();
			// 6. 출력 -> 데이터 저장 후 리턴 -> 데이터 한개
