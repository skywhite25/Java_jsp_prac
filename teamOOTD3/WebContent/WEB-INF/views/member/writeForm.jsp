...
@charset "utf-8";
*{margin:0;padding:0;}
body,button,input,select,table,textarea{width:100%;height:100%;font-family:Dotum,'돋움',Helvetica,sans-serif;font-size:14px;}
#wrap{width:100%;height:1030px;margin:0 0;background-color:#f5f6f7;}
...
$(function(){ // jquery에서 익명함수를 전달해서 저장해놨다가 Document가 로딩이 다되면 호출해서 처리해준다.
    ...
    var bt_success = "";
    $("#yy, #mm, #tel2, #tel3").keyup(function(){
    	...
    	if(yy_val.length < 4 || mm_val.length < 1 || tel2_val.length < 4 || tel3_val.length < 4){
    		$("#checkbt").text(bt_error).addClass("alert-warning");
    		return false;
    	}
   		$("#checkbt").text(bt_success).hide();
   		return true;
    });
});
</script>
		...
		<form action="write.do" id="writeForm" method="post">
			...
			<div class="form-group">
				<label for="birth">생년월일</label><br>
					<input type="datetime" id="yy" name="birth" maxlength="4" placeholder="년(4자)" autocomplete="off" pattern="[0-9]+">
					...
					<input type="datetime" id="dd" name="birth" maxlength="2" placeholder="일" autocomplete="off" pattern="[0-9]+">
			</div>
			<div class="form-group">
				<label for="tel">전화번호</label><br>
					...
					<input type="datetime" id="tel2" name="tel" size="4" maxlength="4" class="form-control" autocomplete="off" pattern="[0-9]+">
					<input type="datetime" id="tel3" name="tel" size="4" maxlength="4" class="form-control" autocomplete="off" pattern="[0-9]+">
				<div id="checkbt" class="alert alert-warning">태어난 년도와 전화번호를 정확하게 입력하세요.</div>
			</div>
			<div class="form-group">
				<label for="email">이메일</label>
				<input type="email" id="email" name="email" maxlength="20" class="form-control" required="required">
			</div>
