...
public class TimelineController implements Controller {
	...
	public String execute(HttpServletRequest request)throws Exception {
		...
		case "/" + MODULE +"/updateFile.do":
			...
			jspInfo = "redirect:view.do?" + query + "&no=" + viewNo;			
			break;
	...
	private Long view(HttpServletRequest request) throws Exception{
		...
		TimelineVO vo = (TimelineVO)ExeService.execute(Beans.getService(AuthorityFilter.url),
				new Long[]{no,inc});
...
//			String strPerPageNum = multi.getParameter("perPageNum");
			...
			if(result ==0 ) throw new Exception("TimelineController - 이미지 게시판 글삭제 오류 - 존재하지 않는 글은 삭제할 수 없습니다.");
