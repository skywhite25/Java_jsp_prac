...
public class PageObject {
	...
	public PageObject() {
		...
		// service 프로그램에서 setTotalRow(DB의 데이터 갯수) 호출을 해야만한다. 그래야 자동 계산이 된다.
	...
	// 객체로 만들어주는 메소드 -> 웹프로젝트의 request객체를 이용
	public static PageObject getInstance(HttpServletRequest request) throws Exception{
		...
		// 넘어온 데이터 확인
		System.out.println( "BoardController.getInstance() [page = " 
		+ strPage + ", perPageNum = " + strPerPageNum+ "]");
		...
		// PageObject - 확인
		System.out.println("BoardController.getInstance() [pageObject = " + pageObject + " ]");
