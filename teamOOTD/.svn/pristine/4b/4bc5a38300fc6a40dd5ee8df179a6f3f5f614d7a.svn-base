...
public class LikeDAO {
	...
	public long fashionCnt(long no) throws Exception{
		...
		try {
			...
			// TODO: handle exception
			e.printStackTrace();
			throw new Exception("fashion cnt 실행 중 DB오류 ");
		}finally {
			DBInfo.close(con, pstmt,rs);
		}
	...
	//1-4.패션 좋아요 취소 
	public int fashionDelete(LikeVO vo) throws Exception{
		int result = 0;
		try {
			con = DBInfo.getConnection();
	...
	//2-2.타임라인좋아요 갯수
		public long timeCnt(long no) throws Exception{
			//결과를 저장해서 보여줘야 하는 변수 
			long result = 0;
			
			try {
				con = DBInfo.getConnection();
				pstmt = con.prepareStatement(DBSQL.TIME_CNT);
				pstmt.setLong(1, no);
				rs = pstmt.executeQuery();
				System.out.println("타임라인 좋아요 갯수 세기 성공");
				if(rs!=null && rs.next()) {
					result = rs.getLong(1);
				}
				...
				// TODO: handle exception
				e.printStackTrace();
				throw new Exception("timeline cnt 실행 중 DB오류 ");
			}finally {
				DBInfo.close(con, pstmt,rs);
			}
			System.out.println(no);
	...
	public int timeLike(LikeVO vo) throws Exception{
		...
		try {
			...
			pstmt.setString(2, vo.getId());		
			result = pstmt.executeUpdate();
			System.out.println("타임라인 좋아요 클릭 성공");
		} catch (Exception e) {
			// TODO: handle exception
			e.printStackTrace();
			throw new Exception("time like 실행 중 DB오류 ");
	...
	//2-4.타임라인 좋아요 취소 
	public int timeDelete(LikeVO vo) throws Exception{
		int result = 0;
		try {
			con = DBInfo.getConnection();
			pstmt = con.prepareStatement(DBSQL.TIME_DELETE);
			pstmt.setLong(1,vo.getNo());		
			pstmt.setString(2,vo.getId());		
			result = pstmt.executeUpdate();
			if(result==1)
				System.out.println("타임라인 좋아요 취소 성공");
