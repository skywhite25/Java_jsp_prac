...
public class MemberController implements Controller{
	...
	public String execute(HttpServletRequest request) throws Exception{
		...
		case "/" + MODULE +"/gradeModify.do":
			// DB에서 입력한 데이터가 맞는지 확인한다.
			gradeModify(request);
			// div안에 들어갈 코드만 있는 jsp로 이동시킨다.
			jspInfo = "redirect:list.do?page=" + pageObject.getPage() + "&perPageNum="
			+ pageObject.getPerPageNum();
			break;
			// 9-2. 상태변경 처리
		case "/" + MODULE +"/statusModify.do":
			// DB에서 입력한 데이터가 맞는지 확인한다.
			statusModify(request);
			// div안에 들어갈 코드만 있는 jsp로 이동시킨다.
			jspInfo = "redirect:list.do?page=" + pageObject.getPage() + "&perPageNum="
			+ pageObject.getPerPageNum();
			break;
	...
	private void gradeModify(HttpServletRequest request) throws Exception {
		...
		LoginVO vo = new LoginVO();
		vo.setId(id);
		vo.setGradeNo(Integer.parseInt(strGradeNo));
		
		// DB처리 -> 등급 변경을 처리한다..
		ExeService.execute(Beans.getService(AuthorityFilter.url), vo);

		session.setAttribute("msg", id + " 회원의 등급이 " + ((vo.getGradeNo() == 9?"관리자로":"일반회원으로"))
				+ " 변경되었습니다.");
		}
	private void statusModify(HttpServletRequest request) throws Exception {
		// 넘어오는 아이디 받기
		String id = request.getParameter("id");
		...
		session.setAttribute("msg", id + " 회원의 상태가 " + status
				+ " 상태로 변경되었습니다.");
