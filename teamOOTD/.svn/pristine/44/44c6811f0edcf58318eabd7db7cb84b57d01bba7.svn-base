...
//답변하기 1증가
	...
	= " select m.id, m.pw, m.name, m.gradeNo, g.gradeName from member m, grade g "
	+ " where (m.id = ? and m.pw = ? and status = '정상') and (m.gradeNo = g.gradeNo) ";
	...
	= "select m.id, m.pw, m.name, m.alias, m.gender, "
	...
	= " update member set name = ?, alias = ?, gender = ?, birth = ?, tel = ?, email = ? where id = ? ";
	public static final String MEMBER_GRADENO_MODIFY
	= "update member set gradeNo = ? where id = ?";
	public static final String MEMBER_STATUS_MODIFY
	= "update member set status = ? where id = ?";
	public static final String MEMBER_CHANGE_PW
	...
	= " select t.no, t.title, t.content, m.id, "
			+ " to_char(t.writeDate, 'yyyy.mm.dd') writeDate, t.fileName, t.hit "
			+ " from timeline t, member m "
	...
	= " update timeline set title = ?, content = ? where no = ? ";
	...
	= "select rnum, rno, no, content, id,"
		...
		= " insert into timeline_reply(rno, no, content, id) "
				+ " values(timeline_reply_seq.nextval, ?, ?, ?) ";
		// 9. 댓글 수정
	...
	=" SELECT ID FROM FASHION_LIKE WHERE NO = ? AND ID = ? ";
	...
	=" SELECT ID FROM TIME_LIKE WHERE NO = ? AND ID = ?";
	
	public static final String TIME_LIKE
	=" INSERT INTO TIME_LIKE (NO,ID) VALUES (?,?) ";
	
	public static final String TIME_CNT
	=" SELECT COUNT(*) CNT FROM TIME_LIKE WHERE NO =? ";
	
	public static final String TIME_DELETE
	=" DELETE FROM TIME_LIKE WHERE NO =? AND ID=?";
	...
	= "SELECT COUNT(*) FROM fashion WHERE seasonNo = ?";
