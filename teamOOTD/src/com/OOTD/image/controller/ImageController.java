...
public class ImageController implements Controller{
	...
	public String execute(HttpServletRequest request) throws Exception {
		...
		case "/" + MODULE + "/replyWrite.do":
			replyWrite(request);
			System.out.println("ImageController [query] : " + request.getQueryString());
			// "view.do"로 넘긴다.
	...
	private void list(HttpServletRequest request, PageObject pageObject) throws Exception {
		...
		System.out.println("ImageController.list [seasonNo] : " + strSeasonNo);
		
		// 현재 날짜 객체 만들기
		Calendar today = Calendar.getInstance();
		// 월 데이터 가져오기
		int month = today.get(Calendar.MONTH);
		System.out.println("ImageController. list [month]: " + month);
		...
		if(strSeasonNo != null)
			seasonNo = Integer.parseInt(strSeasonNo);
		System.out.println("ImageController.list [seasonNo] : " + seasonNo);
		pageObject.setSeasonNo(seasonNo);
	...
	// 2. 이미지게시판 글보기 
	public Long view(HttpServletRequest request) throws Exception {
		// 넘어오는 데이터 받기 - 번호(, 페이지, 한 페이지당 이미지 갯수 : 나중에 처리)
		String strNo = request.getParameter("no");
		System.out.println("ImageController. view [no] :" + strNo);
...
//		String strPerPageNum = multi.getParameter("perPageNum");
	...
	// 처리되는 정보를 출력하는 프록시 구조의 프로그램을 거쳐가서 정보가 출력이 된다.
	request.setAttribute("list", ExeService.execute(Beans.getService("/image/replyList.do"), new Object[] {no, pageObject}));
	...
	// 9. 게시판 등록 수정 처리
	private long replyUpdate(HttpServletRequest request) throws Exception {
		System.out.println("/image/replyUpdate.jsp - 댓글수정 처리 완료");
		...
		System.out.println("ImageController.replyUpdate [rno] : " + rno);
		...
		System.out.println("/image/replyUpdate.jsp [vo] : " + vo);

		Integer result = (Integer) ExeService.execute(Beans.getService(AuthorityFilter.url), vo);
		
		if(result < 1)
			throw new Exception("게시판 댓글수정(ImageController.replyUpdate() - 수정할 데이터가 존재하지 않습니다.");
	...
	public void replyDelete(HttpServletRequest request) throws Exception {
		System.out.println("/image/replyDelete.jsp - 댓글삭제 처리 완료");
		// 넘어오는 데이터 받기
		String strRno = request.getParameter("rno");
		Long rno = Long.parseLong(strRno);
		String strNo = request.getParameter("no");
		...
		System.out.println("/image/replyDelete.jsp [no] : " + strNo);
		System.out.println("/image/replyDelete.jsp [rno] : " + strRno);
