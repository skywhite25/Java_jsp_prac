...
// 		alert($("#title").val());
// 		alert($("#content").val());
		// C:\fakepath\flower01.jpg --> 보안상 표시되는 경로
// 		alert($("#imageFile").val());
// 		첨부파일이 이미지 파일인지 알아내는 프로그램 작성 -> 확장자 : 파일명의 마지막 "." 이후의 글자
		var fileName = $("#imageFile").val();
		// substring(시작[, 끝]) -> 부분 문자열 잘라내기
		// lastIndexOf(찾는 문자열) -> 뒤에서 부터 찾는 문자열의 위치. 찾는 문자열이 없으면 -1. -> index번호는 1부터가 아닌 0부터 시작.
...
<div class="container">
	<h1>이미지 등록</h1>
	<!-- 파일첨부를 하는 입력에는 반드시 post 방식이어야 하고 enctype을 지정해야만 한다.
		input태그의 type="file"로 지정한다. -->
	<form action="write.jsp" method="post" enctype="multipart/form-data" id="writeForm">
		...
		<div class="form-group">
			<label for="imageFile">이미지파일(JPG, JPEG, GIF, PNG - 이미지 지원)</label>
