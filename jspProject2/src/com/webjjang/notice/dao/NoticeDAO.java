...
public class NoticeDAO {
	...
	public List<NoticeVO> list(PageObject pageObject) throws Exception {
		...
		try {
			...
			System.out.println(DBSQL.NOTICE_LIST);
			//4. 실행객체 & 데이터 셋팅
			...
			e.printStackTrace();
			throw new Exception("공지사항 리스트 데이터 처리 중 DB오류가 발생.");
		} finally {
			//7. 닫기
	...
	// 공지사항 전체데이터 갯수 가져오기 -----------------------------
		public long getTotalRow() throws Exception {
			...
			try {
				...
				con = DBInfo.getConnection();
				//3. 쿼리 작성 - DBSQL에 선언됨. - 확인
				System.out.println(DBSQL.NOTICE_GET_TOTALROW);
				//4. 실행객체 & 데이터 셋팅
				...
				e.printStackTrace();
				throw new Exception("공지사항 전체데이터 가져오기 중 DB오류가 발생.");
			} finally {
				//7. 닫기
				DBInfo.close(con, pstmt, rs);
			} // end of try~catch ~finally
	...
	public int write(NoticeVO vo) throws Exception{
		...
		try {
			...
			con = DBInfo.getConnection();
			//3. 쿼리 작성 - DBSQL에 선언됨. - 확인
			System.out.println(DBSQL.NOTICE_WRITE);
			//4. 실행객체 & 데이터 셋팅
